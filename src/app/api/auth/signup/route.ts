 ...
 * 3.  백엔드 응답이 성공적이고 `accessToken`과 `refreshToken`이 포함된 경우,
 * 이를 안전한 `HttpOnly` 쿠키로 설정하여 사용자에게 즉시 로그인 상태를 제공합니다.
 * 4.  백엔드 응답에 토큰이 없는 경우(회원가입만 성공), 로그인 페이지로 이동하라는 메시지를 반환합니다. (부분 성공)
 * 5.  최종적으로 클라이언트에는 사용자 정보(`user`) 또는 에러 메시지를 반환합니다.
 *
 * ### 에러 처리 흐름:
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiResponse.ok) {
      throw Object.assign(
        new Error(responseBody.message || '회원가입에 실패했습니다.'),
        {
