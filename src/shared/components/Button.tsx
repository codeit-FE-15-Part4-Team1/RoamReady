...
const buttonVariants = {
  ...
  sizes: {
    /** 작은 크기 - 콘텐츠에 맞춤, 작은 패딩 */
    small: 'w-fit h-fit px-10 py-5 text-sm',
    /** 중간 크기 - 고정 너비/높이, 일반적인 버튼 크기 */
    medium: 'w-120 h-40 text-base',
    /** 큰 크기 - 전체 너비, 큰 높이 */
    large: 'w-full h-40 text-lg',
  },

  /** 선택된 상태 스타일 - 브랜드 컬러 텍스트와 테두리 */
  selected: 'text-primary-500 border border-primary-500',
 ...
 * @example
 * ```typescript
 * getButtonClasses('primary', 'large', false, false)
 * // returns: 'w-fit h-fit border rounded-2xl flex items-center justify-center w-full h-40 text-lg bg-primary-500 text-white'
 * ```
 */
...
export default function Button({
  ...
  return (
    <button
      {...props}
      className={cn(
        getButtonClasses(variant, size, selected, props.disabled),
        className,
      )}
    >
      {children}
