 ...
 * 이를 통해 z-index 문제를 해결하고 Dialog가 항상 최상위에 표시되도록 보장합니다.
 *
 * **주요 기능:**
 * - React Portal을 통한 DOM 트리 외부 렌더링
 * - Dialog 열림/닫힘 상태에 따른 조건부 렌더링
 ...
 *
 * **스크롤 관리:**
 * - Dialog가 열릴 때: 현재 스크롤 위치를 저장하고 body를 고정
 * - Dialog가 닫힐 때: body 고정을 해제하고 원래 스크롤 위치로 복원
 * - 컴포넌트 언마운트 시: 스크롤 상태 cleanup
 *
 * @param children - Portal을 통해 렌더링될 Dialog 콘텐츠
 *
...
export function DialogPortal({ children }: DialogPortalProps) {
  ...
  /**
   * Dialog 상태에 따른 스크롤 제어
   *
   * Dialog가 열려있을 때 배경 스크롤을 방지하고,
   * 닫힐 때 원래 스크롤 위치를 복원합니다.
   *
   * **동작 순서:**
   * 1. Dialog 열림: 현재 스크롤 위치(scrollY) 저장
   * 2. body를 position: fixed로 고정하고 top: -scrollY로 설정
   * 3. Dialog 닫힘: body 스타일 초기화 후 저장된 위치로 스크롤
  ...
  // Dialog가 닫혀있거나 아직 마운트되지 않은 경우 렌더링하지 않음
